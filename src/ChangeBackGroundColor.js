...
const ChangeBackGroundColor = ({ handleBackgroundColor, activeBackgroundColor }) => {
    return (
        <>
            ...
            <div className="my-3 min-w-0 flex-1 grid grid-cols-5 2xl:grid-cols-10 gap-x-4 gap-y-3 2xl:gap-x-2">
                {colorDefault.map((color, index) => (
                    <button
                        key={color.id} 
                        type='button' 
                        className={`flex items-center justify-center w-full h-10 p-3 rounded bg-${color.value} border border-truegray-200 ${activeBackgroundColor === color.hex ? `ring-2 ring-offset-2 ring-${color.value}` : undefined} cursor-pointer`}
                        value={color.hex} 
